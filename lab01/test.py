...
def test_inverted_2():
    im = {
        'height': 1,
        'width': 4,
        'pixels': [25, 86, 159, 219]
    }
    result = lab.inverted(im)
    expected = {
        'height': 1,
        'width': 4,
        'pixels': [230, 169, 96, 36],
    }
    compare_greyscale_images(result, expected)
...
def test_blurred_black_image():
    ...
    compare_greyscale_images(result1, expected)
    compare_greyscale_images(result2, expected)
...
def test_blurred_centered_pixel():
    ...
    result2 = lab.blurred(im, 5)
    pixels = [0]*13*13
    for element in [48, 49, 50, 59, 60, 61, 70, 71, 72]:
        pixels[element] = round(255/9)
    ...
    compare_greyscale_images(result1, expected1)

    pixels = [0]*11*11
    for element in [36, 37, 38, 39, 40, 47, 48, 49, 50, 51, 58, 59, 60, 61, 62, 69, 70, 71, 72, 73, 80, 81, 82, 83, 84]:
        pixels[element] = round(255/25)
    expected2 = {
        'height': 11,
        'width': 11,
        'pixels': pixels
    }
    compare_greyscale_images(result2, expected2)
